...
if (!isset($_SESSION['adminid']) && !isset($_SESSION['driverid'])) {
    ...
    if (isset($_POST['submit'])) {
        ...
        $remark = $_POST['remark']; 
        $status = $_POST['status'];
     // Get the ambulance reg number from the form

        // Update ambulance and hiring records
        $updateQuery1 = mysqli_query($con, "UPDATE tblambulance SET status='$status'");
        $updateQuery2 = mysqli_query($con, "UPDATE tblambulancehiring SET Status='$status', Remark='$remark', WHERE BookingNumber='$bookingnum'");
        
        // Insert tracking history
        $insertQuery = mysqli_query($con, "INSERT INTO tbltrackinghistory(BookingNumber,  Remark, Status) VALUES ('$bookingnum', '$remark', '$status')");
